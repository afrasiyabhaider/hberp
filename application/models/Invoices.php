...
// count invoice search by customer
    ...
    public function invoice_entry() {
        $invoice_id = date('Ymd').time(); //$this->generator(10);
        ...
        if (($this->input->post('customer_id') == null ) && ($this->input->post('customer_name') == null )) {
            ...
            foreach ($query->result() as $row) {
                $json_customer[] = array('label' => $row->customer_name.' - '.$row->customer_location, 'value' => $row->customer_id);
            }
        ...
        $datainv = array(
            ...
            'showBalance' => $this->input->post('showBalance'),
...
//        echo '<pre>';        print_r($customer_ledger); 
    ...
    public function get_total_product($product_id, $supplier_id) {
        ...
        $salesData = $this->invoice_data($product_id);
        ...
        $available_quantity = ($total_purchase->total_purchase - $totalSales); //$total_sale->total_sale
