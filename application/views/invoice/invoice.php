                        ...
                        <div class="table-responsive">
                            <table id="dataTableExample6" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    ...
                                    if ($invoices_list) {
                                        ?>
                                        <?php foreach($invoices_list as $il){?>
                                        <tr>
                                            <td><?=$il['sl']?></td>
                                            <td>
                                                <a href="<?php echo base_url() . 'Cinvoice/invoice_inserted_data/'.$il['invoice_id']; ?>">
                                                   <?=$il['invoice']?>
                                                </a>
                                            </td>
                                            <td>
                                                <a href="<?php echo base_url() . 'Ccustomer/customerledger/'.$il['customer_id']; ?>"><?=$il['customer_name']?></a>				
                                            </td>

                                            <td><?=$il['final_date']?></td>
                                            <td style="text-align: right;"><?php echo $currency; if($il['gTotal'] != "0.00") { echo $il['gTotal']; $g = $il['gTotal'];}else{ echo $il['total_amount']; $g = $il['total_amount'];} ?></td>
                                            <td style="text-align: right;"><?php echo (($position == 0) ? "$currency ".$il['paidamount'] : $il['paidamount']." $currency") ?></td>
                                            <td style="text-align: right;"><?php echo (($position == 0) ? "$currency ".($g - $il['paidamount']) : ($g - $il['paidamount'])." $currency") ?></td>
                                            <td>
                                    <center>
                                        <?php echo form_open() ?>

                                        <a href="<?php echo base_url() . 'Cinvoice/invoice_inserted_data/'.$il['invoice_id']; ?>" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="left" title="<?php echo display('invoice') ?>"><i class="fa fa-window-restore" aria-hidden="true"></i></a>


                                        <a href="<?php echo base_url() . 'Cinvoice/pos_invoice_inserted_data/'.$il['invoice_id']; ?>" class="btn btn-warning btn-sm" data-toggle="tooltip" data-placement="left" title="<?php echo display('pos_invoice') ?>"><i class="fa fa-fax" aria-hidden="true"></i></a>

                                        <a href="<?php echo base_url() . 'Cinvoice/invoice_update_form/'.$il['invoice_id']; ?>" class="btn btn-info btn-sm" data-toggle="tooltip" data-placement="left" title="<?php echo display('update') ?>"><i class="fa fa-pencil" aria-hidden="true"></i></a>

                                        <a href="" class="deleteInvoice btn btn-danger btn-sm" name="<?=$il['invoice_id']?>" data-toggle="tooltip" data-placement="right" title="" data-original-title="<?php echo display('delete') ?> "><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                                        <?php echo form_close() ?>
                                    </center>
                                    </td>
                                    </tr>
