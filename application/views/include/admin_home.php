...
//                                            echo '<pre>';   print_r($todays_sales_report); echo '<pre>';
                                            foreach ($todays_sales_report as $single) {
                                                $sql = "SELECT (SELECT SUM(total_price) FROM invoice_details a JOIN invoice b ON b.invoice_id = a.invoice_id WHERE a.invoice_id = '" . $single->invoice_id . "' AND b.customer_id = '" . $single->customer_id . "') as total_amount, 
														...
														(SELECT due_amount FROM invoice_details a JOIN invoice b ON b.invoice_id = a.invoice_id WHERE a.invoice_id = '" . $single->invoice_id . "' AND b.customer_id = '" . $single->customer_id . "' group by a.invoice_id) as total_due, 
...
//                                               echo '<pre>'; print_r($result);
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-right">
                                                        <?php
                                                        $ttl_paid += $todays_receipt_result->today_receipt;//$result->total_paid;
                                                        echo number_format($todays_receipt_result->today_receipt, '2', '.', ',');//number_format($result->total_paid, '2', '.', ','); ?>
                                                    </td>
                                                    <td class="text-right">
                                                        <?php
