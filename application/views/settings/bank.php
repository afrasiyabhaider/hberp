		                    ...
		                    <table id="dataTableExample3" class="table table-bordered table-striped table-hover">
								...
								{bank_list}
									<tr>
										...
										<?php echo form_close()?>
										<a href="" class="deleteBank btn btn-danger btn-sm" name="{bank_id}" data-toggle="tooltip" data-placement="right" title="" data-original-title="<?php echo display('delete') ?> "><i class="fa fa-trash-o" aria-hidden="true"></i></a>
										</td>
									</tr>
								{/bank_list}
								<?php
...
<script type="text/javascript">
    ...
    {
        var Bank_id = $(this).attr('name');
        var x = confirm("Are You Sure,Want to Delete ?");
        if (x == true) {
            $.ajax
                    ({
                        type: "POST",
                        url: '<?php echo base_url('Csettings/bank_delete1/') ?>'+Bank_id,
