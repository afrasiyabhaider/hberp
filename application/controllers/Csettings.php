...
class Csettings extends CI_Controller {
    ...
    public function manage_person() {
        ...
        $config["per_page"] =   $this->Settings->person_list_count(); //10;
    ...
    public function person_delete() {
        ...
        $this->Settings->delete_person($person_id);
        $this->session->set_userdata(array('message' => display('successfully_delete')));
        return true;
    }
    
    public function person_delete1($person_id) {
        $this->load->model('Settings');
        $this->Settings->delete_person($person_id);
        $this->session->set_userdata(array('message' => display('successfully_delete')));
        return true;
    }
    public function pesonal_delete1($pesonal_id) {
        $this->load->model('Settings');
        $this->Settings->delete_pesonal($pesonal_id);
        $this->session->set_userdata(array('message' => display('successfully_delete')));
        return true;
    }
    public function bank_delete1($bank_id) {
        $this->load->model('Settings');
        $this->Settings->delete_bank($bank_id);
        $this->session->set_userdata(array('message' => display('successfully_delete')));
        return true;
    }
