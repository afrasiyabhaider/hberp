...
class Cinvoice extends CI_Controller {
    ...
    public function insert_invoice() {
        ...
        $this->pos_invoice_inserted_data($invoice_id);
    }
    ...
    public function invoice_update() {
        ...
        $this->pos_invoice_inserted_data($invoice_id);  //invoice_inserted_data
    }
    ...
    public function manage_invoice() {
        ...
        $config["per_page"]   = $this->Invoices->invoice_list_count(); 
    ...
    public function date_to_date_invoice() {
        ...
        $config["per_page"] = $this->Invoices->invoice_list_date_to_date_count($from_date, $to_date);;
        $config["uri_segment"] = 3;
        $config["num_links"] = 5;
        $config['suffix'] = '?' . http_build_query($_GET, '', '&');
        $config['first_url'] = $config["base_url"] . $config['suffix'];
    ...
    public function insert_pos_invoice() {
        ...
        if (!empty($product_details)) {
            ...
            $tr .= "<tr id=\"row_" . $product_id . "\">
                        ...
                        <td>
                            <input type=\"number\" name=\"product_quantity[]\" onkeyup=\"quantity_calculate('" . $product_id . "');\" onchange=\"quantity_calculate('" . $product_id . "');\" class=\"total_qntt_" . $product_id . " form-control text-right\" id=\"total_qntt_" . $product_id . "\" placeholder=\"0.00\" min=\"0\" value=\"1\"/>
                        </td>
...
//    =========== its for 1 increment =============
    ...
    public function getBlance($customer){
        
        $sql   = 'SELECT * from customer_ledger where customer_id = "'.$customer.'" order by id ASC';
